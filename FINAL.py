...
class PortScannerGUI:
    def __init__(self, root):
        ...
        # Styling for the entire window
        self.root.style = ttk.Style()
        self.root.style.configure("TButton", background="green", foreground="black")

        #  target IP/Domain input
        self.label_host = ttk.Label(root, text="Target IP/Domain:")
        self.label_host.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        ...
        #  server ports input
        self.label_ports = ttk.Label(root, text="Server Port(s) (Example- 80,124 or leave empty to scan all ports):")
        self.label_ports.grid(row=1, column=0, padx=10, pady=10, sticky="w")
        ...
        self.button_scan = ttk.Button(root, text="Scan", command=self.scan_ports, style="TButton")
        self.button_scan.grid(row=0, column=2, padx=10, pady=10, rowspan=2, sticky="nsew")
        ...
        self.text_output = tk.Text(root, height=20, width=70, background='#808080', foreground='black')
    ...
    def run_scanner(self, host, start_port, end_port):
        ...
        threads = 2048  # Number of threads for scanning
        ...
        if 443 in open_ports_tcp :
            self.text_output.insert("end", "TCP Port 443 is open!\nThe domain is secure and is using SSL.\n")
        else:
            self.text_output.insert("end", "TCP Port 443 is not open.\nThe domain may not be using SSL.\n")

        self.text_output.insert("end", f"\nScanning complete at: {datetime.now()}\n")

        self.text_output.insert("end", f"\n\nOPEN TCP PORTS ARE : {len(open_ports_tcp)}\n\n")
        self.text_output.insert("end", f"\n\nOPEN UDP PORTS ARE : {len(open_ports_udp)}\n\n")
